...
In the same emission, list the artifact under `artifacts:` (`.sdlc/context/roadmap.md`).
...
A review subagent is dispatched automatically to run `/review-roadmap` to audit the roadmap for alignment, sequencing, focus, horizon discipline, and currency before relying on it for prioritization.
