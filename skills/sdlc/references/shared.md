...
the orchestrator (`sdlc`), the setup skills (`initialize-sdlc-directory`, `sync-sdlc`, `update-sdlc-templates`, `sdlc-status`, `propagate-changes`, `audit-sdlc`), every `create-*` and `review-*` pipeline skill, and any skill that consults `.sdlc/context/` for project context.
...
- `sync-meta.yml` and generated reports (`audit-report.md`, `propagation-report.md`, `improvement-dryrun-*.md`) stay in the repo's `.sdlc/` only.

## Concurrent `.sdlc/` Modifications

The `.sdlc/` tree is shared mutable state.
Multiple sessions (interactive sessions, scheduled tasks, automation runners, other agents) may read and write it at the same time, so anything read earlier in a session may have changed by the time a skill acts on it.

At each point where a skill executes a concrete SDLC step, re-read the `.sdlc/` files that step depends on instead of relying on an earlier read:

- Before drafting or amending an artifact: re-read the target artifact, its `review-<artifact>.md` findings (if any), the sibling artifacts it cross-references, and the `.sdlc/context/` files it applies.
- Before deciding revision mode: re-read the findings file; another session may have written or resolved it since the run started.
- Before creating a feature directory or knowledge record: re-run the create-time / next-sequence check against the current directory listing.
...
artifacts:               # optional; every output file this skill produced or updated
  - <repo-relative path>
```
...
- `propagate-changes` and `sync-sdlc` may regress an approved findings file to `verdict: changes-requested`, recording the drift in the body, when they detect the reviewed artifact has drifted from the code. The forward pipeline then resyncs the artifact via revision mode (see below) and the matching `review-*` skill restores `approved` once the drift is resolved.
