...
2. If `.sdlc/` does not exist, run `/initialize-sdlc-directory` (passing `$1` if provided) to create the directory tree and copy templates.
...
3. Read existing `.sdlc/` content to establish the current state:
...
4. Analyze the codebase to gather the information needed to fill in context files and identify features.
...
5. Reconcile and write `.sdlc/context/project-overview.md`.
   ...
   - If the file exists: read it, compare its content with the codebase analysis, and update sections that have drifted. Preserve any manual additions that are still accurate. Flag sections that changed in the report.
...
6. Reconcile and write `.sdlc/context/architecture.md` using the same compare-and-update approach.
...
7. Reconcile and write `.sdlc/context/conventions.md` using the same compare-and-update approach.
...
8. Reconcile and write `.sdlc/context/vocabulary.md` using the same compare-and-update approach.
...
9. Identify the major features or subsystems of the project.
...
10. Match identified features against existing `.sdlc/features/` directories.
    Build three lists:
